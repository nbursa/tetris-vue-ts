...
export default defineComponent({
  ...
  setup() {
    ...
    // const moveBlock = () => {
    //   console.log('lastMoveTime.value:', lastMoveTime.value)
    //   // if (Date.now() - lastMoveTime.value > moveInterval) {
    //   // }
    ...
    const moveBlock = () => {
      ...
      if (currentTime - lastMoveTime.value >= moveInterval) {
        if (!willCollide(blockPosition.value.x, blockPosition.value.y + 1)) {
          blockPosition.value.y += 1;  // Move the block down
          lastMoveTime.value = currentTime;  // Update the time of the last move
          console.log("blockPosition.value.y:", blockPosition.value.y)
        } else {
          placeBlock();
          resetBlock();
        }
        nextFrame();  // Schedule the next frame
      } else {
        nextFrame();  // Continue animation until it's time to move
      }
    };
    ...
    //     if (!willCollide(blockPosition.value.x, blockPosition.value.y + 1)) {
    //       blockPosition.value.y += 1;
    //       lastMoveTime.value = Date.now();  // Update last move time
    //     } else {
    //       placeBlock();
    //       resetBlock();
    //     }
    ...
    //   moveBlock();
    //   if (!willCollide(blockPosition.value.x, blockPosition.value.y + 1)) {
    //     requestAnimationFrame(animate);  // Continue animation until a block collides
    //   }
    // };
    const nextFrame = () => {
