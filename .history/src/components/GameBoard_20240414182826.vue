...
export default defineComponent({
  ...
  setup() {
    ...
    const moveBlock = () => {
      ...
      if (currentTime - lastMoveTime.value >= moveInterval) {
        if (!willCollide(blockPosition.value.x, blockPosition.value.y + 1)) {
          columns.value[blockPosition.value.x][blockPosition.value.y] = 0; // Clear the previous position
          blockPosition.value.y += 1; // Move the block down
          columns.value[blockPosition.value.x][blockPosition.value.y] = 1; // Set the new position
          columns.value = [...columns.value]; // Update the columns to trigger reactivity
    ...
    const nextFrame = () => {
      ...
      if (!willCollide(blockPosition.value.x, blockPosition.value.y)) {
        nextFrame(); // Start the game unless the initial position is colliding
      } else {
        alert('Game Over!');
      }
    };
