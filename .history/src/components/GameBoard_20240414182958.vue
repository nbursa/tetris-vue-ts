    ...
    const moveBlock = () => {
      ...
      if (currentTime - lastMoveTime.value >= moveInterval) {
        if (!willCollide(blockPosition.value.x, blockPosition.value.y + 1)) {
          columns.value[blockPosition.value.x][blockPosition.value.y] = 0;
          blockPosition.value.y += 1;
          columns.value[blockPosition.value.x][blockPosition.value.y] = 0;  // Reset the previous position
          columns.value[blockPosition.value.x][blockPosition.value.y - 1] = 1;  // Set the new position
          // columns.value = columns.value.slice();  // Ensure the update is reactive
          columns.value = [...columns.value];
