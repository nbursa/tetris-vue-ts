...
/
    ...
    const willCollide = (x: number, y: number) => {
      if (y >= columns.value[x].length) return true; // Out of bounds check
      return columns.value[x][y] !== 0; // Check if cell is already filled
    };
    ...
    const isBlock = (columnIndex: number, rowIndex: number): boolean => {
      return columns.value[columnIndex][rowIndex] !== 0; // Check if cell is filled
    };
